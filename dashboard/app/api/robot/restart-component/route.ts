...
const COMPONENT_COMMANDS: Record<string, { kill: string; start: string }> = {
  ...
  slam: {
    ...
    start: `${ROS_ENV} && nohup rosrun gmapping slam_gmapping _base_frame:=base_footprint _odom_frame:=odom _map_update_interval:=1.0 _maxUrange:=6.0 _maxRange:=8.0 _particles:=30 _linearUpdate:=0.1 _angularUpdate:=0.15 _temporalUpdate:=2.0 _delta:=0.03 _xmin:=-10.0 _xmax:=10.0 _ymin:=-10.0 _ymax:=10.0 _minimumScore:=50 > /tmp/restart_slam.log 2>&1 &`,
  },
  jetracer: {
